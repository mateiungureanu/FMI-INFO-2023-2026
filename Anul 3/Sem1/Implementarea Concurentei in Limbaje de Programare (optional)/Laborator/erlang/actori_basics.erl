-module(actori_basics).

-export([main/1, fibo/1, pbThread/0, main2/1]).
...
pbThread() ->
  receive
    {map, plus_1, L, From} ->
      ...
      io:format("Rezultat map + 1: ~p de la ~p ~n", [NewList, From]),
      pbThread();
    {filter, odd, L, From} ->
      NewList = [N || N <- L, N rem 2 =:= 1],
      io:format("Rezultat filter odd: ~p de la ~p ~n", [NewList, From]),
      pbThread();
    {filter, even, L, From} ->
      NewList = [N || N <- L, N rem 2 =:= 0],
      io:format("Rezultat filter even: ~p de la ~p ~n", [NewList, From]),
      pbThread();
    {sum, L, From} ->
      Res = lists:foldr(fun(X, Y) -> X + Y end, 0, L),
      io:format("Rezultat sum: ~p de la ~p ~n", [Res, From]),
      pbThread();
    {prod, L, From} ->
      Res = lists:foldr(fun(X, Y) -> X * Y end, 1, L),
      io:format("Rezultat prod: ~p de la ~p ~n", [Res, From]),
      pbThread();
    _ ->
      io:format("Comanda invalida lol~n"),
      pbThread()
  end,
  timer:sleep(500).
