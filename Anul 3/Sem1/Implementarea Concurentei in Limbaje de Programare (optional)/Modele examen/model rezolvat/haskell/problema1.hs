...
{- 
    ...
    - si prietenii sai care sunt workers: reni si elfi

    Programul are 10 threaduri elf, 9 threaduri ren, 1 thread principal Santa. 
    elfii si renii trebuie sa formeze grupuri de o anumita capacitate.

    dupa ce s a realizat o grupare, e preluata de Santa. 
    - renii au prioritate
    ...
    ciclul de viata pt un elf sau ren (individual):
    ...
    Dupa ce grupul s a format, primeste 2 porti, una de intrare si una de iesire
...
-- reamintim ca scrierea la stdout nu e sincronizata
-- deci trebuie sa o facem manual

-- folosim MVar ca mutex
-- folosim writeStdOut ca sa fie thread-safe
-- takeMVar -> acquire, putMvar -> release
...
data Gate = MkGate Int (TVar Int)
                        -- nr de chei dispobile la un mom dat
...
santa stdw elf_group reindeer_group = do
    (task, (in_gate, out_gate)) <- atomically $ orElse
        (chooseGroup reindeer_group "deliver toys")
        (chooseGroup elf_group "meet in my study")
...
main = do
    stdw <- newMVar ()
    elf_group <- newGroup 1
    sequence_ [elf elf_group n stdw | n <- [1..5]]

    reindeer_group <- newGroup 10
    sequence_ [reindeer reindeer_group n stdw | n <- [1..50]]

    forever $ santa stdw elf_group reindeer_group