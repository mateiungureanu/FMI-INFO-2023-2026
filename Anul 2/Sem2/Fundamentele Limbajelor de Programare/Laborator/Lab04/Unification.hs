...
unifyStep (Config (Var x :=: Var y : eqs) subst)
    -- Regula 'Scoate'
    | x == y = Right $ Config eqs subst
    -- nu e necesar sa facem si cazul otherwise, vo match-ui automat cu urmatorul caz posibil
unifyStep (Config (Var x :=: t : eqs) subst) 
    -- Caz 1: avem ciclu, deci esec
    | x `elem` vars t = Left $ "Cycle on " ++ x
    -- Caz 2: nu avem ciclu, aplicam regula 'Rezolva':
    -- "singletonSubst x t" == x |-> t
    -- "eapply s e" => aplica substitutia s peste cei doi termeni ai ecuatiei e
    -- "extend subst x t" == subst = (x |-> t)
    | otherwise = Right $ Config (map (eapply (singletonSubst x t :: SubstitutionGraph)) eqs) (extend subst x t)
unifyStep (Config (t :=: Var x : eqs) subst)
    -- identic
    | x `elem` vars t = Left $ "Cycle on " ++ x
    | otherwise = Right $ Config (map (eapply (singletonSubst x t :: SubstitutionGraph)) eqs) (extend subst x t)
unifyStep (Config (App f1 args1 :=: App f2 args2 : eqs) subst)
    -- Caz 1: conflict de functii, deci esec
    | f1 /= f2 || length args1 /= length args2 = Left $ "Conflict between " ++ f1 ++ "/" ++ show (length args1) ++ " and " ++ f2 ++ "/" ++ show (length args2)
    -- Caz 2: cele doua functii sunt identice, aplicam regula 'Descompune'
    -- "zipWith: aplica o functie arbitrara pe perechile de elemente de pe pozitii egale din args1 si args2 si intoarce lista rezultata
    -- Nota: zip == zipwith (,)
    | otherwise = Right $ Config (zipWith (:=:) args1 args2 ++ eqs) subst
...
instance HasUnify [Equation] where
  unify eqs = go (Config eqs idSubst)
    where
      ...
      -- in loc de bind monadic >>=
      -- go cfg = do
      --   newConfig  <- unifyStep cfg
      --   go newConfig
