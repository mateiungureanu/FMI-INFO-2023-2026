...
import Substitution (capply, SubstitutionGraph (SubstitutionGraph), Substitution (Substitution), makeGraph, SubstitutionLike (idSubst), qapply, (>>>), restrict)
...
resolutionStep (Query (fst:qs)) clause = do
  theta <- unify [fst, clHead freshClause]
  Right (theta, qapply theta (Query (clBody freshClause ++ qs)))
  where freshClause = refreshWrt clause fst
resolutionStep _ _ = Left "This should not happen"
...
WAS Right (Y_1 |-> Y, X |-> powerModule(X_1),verifyComponent(X_1), matchResource(Y, X_1))
NOW Right (Y |-> Y_1, X |-> powerModule(X_1),verifyComponent(X_1), matchResource(Y_1, X_1))
...
WAS Right (X_2 |-> X_1, X |-> powerModule(X_1), Y_1 |-> Y,combineElements(craftToken(Y_2), X_1), matchResource(Y_2, Y_2), matchResource(Y, X_1))
NOW Right (X_1 |-> X_2, X |-> powerModule(X_2), Y_1 |-> Y,combineElements(craftToken(Y_2), X_2), matchResource(Y_2, Y_2), matchResource(Y, X_2))
