...
--v2
select last_name, salary,k.dep_id,k.department_name,  round(k.medie), k.nr_ang
from employees e,
    (select  dd.department_id dep_id, dd.department_name, count(employee_id) nr_ang, (avg(salary)) medie
    from employees ee, departments dd
    where dd.department_id = ee.department_id
    group by dd.department_id, dd.department_name) k
where e.department_id = k.dep_id
...
from employees e join 
    ...
    where dd.department_id = ee.department_id
    group by dd.department_id, dd.department_name) k
on( e.department_id = k.dep_id)
where e.salary > k.medie ; --38 rez
...
9-1. Sã se determine departamentele(numele) în care exista un angajat.
...
select * 
from subord_directi
where hire_date = (select maxim from vechime_maxima); --!!!atentie ;a tabela vechime_maxima
...
Care sunt top 10 salarii cele mai mari si cine este platit cu acele salarii?
...
select * 
from employees
where salary in ( select salary from sal_10); --19 rez
...
--top 7 salarii si angajatii care le detin
...
with sal_14 as
(select *
from (Select distinct avg(salary) medie
      from employees  
      group by job_id
      order by medie asc
) where rownum <=14)
select job_id, avg(salary) med
from employees 
group by job_id
having avg(salary) in (select medie from sal_14) -- nu ppot folosi aliasurile din select : having med in (...)
order by avg(salary); --15 rez -- aici pot folosi: order by med;

--!!! in subcerere nu putem folosi ordonarea
select *
from employees
where salary in (Select distinct salary
                    from employees);
                  --  order by 1 desc); --!!!!!! NUUUUUU!!!!
                  
--in with putem sa folosim ordonarea               
with ang as (Select distinct salary
                    from employees
                    order by 1 desc)
select *
from employees
...
Select distinct sum(salary) medie
from employees  
group by job_id
order by medie asc; --18 rez

Select sum(salary) medie
from employees  
group by job_id
order by medie asc; --19 rez

Select distinct sum(salary) medie, job_id
from employees  
group by job_id
order by medie asc; --19 rez

--dar angajatii care lucreaza pe aceste job-uri?
...
--pt dep cu sal max din firma, hire date ul ultimului angajat
--pt dep care are m, nr de ang
--altfel, salariul minim din firma

select department_id,
case
when lower(department_name) like '%m%'
then (select to_char(count(employee_id)) from employees e2 where e2.department_id = department_id)
when department_id =
(select department_id from employees e where e.salary =
(select max(salary) from employees)) then (select to_char(max(hire_date)) from employees e where
e.department_id = d.department_id)
else (select to_char(min(salary)) from employees)
end x
from departments d;

SELECT department_id, department_name, CASE WHEN UPPER(department_name) LIKE '%M%' THEN TO_CHAR((SELECT COUNT(employee_id) FROM employees))
WHEN department_id = (SELECT department_id FROM employees WHERE salary = (SELECT MAX(salary) FROM employees)) THEN TO_CHAR((SELECT MAX(hire_date) FROM employees))
ELSE TO_CHAR((SELECT MIN(salary) FROM employees)) END detalii
FROM departments;
