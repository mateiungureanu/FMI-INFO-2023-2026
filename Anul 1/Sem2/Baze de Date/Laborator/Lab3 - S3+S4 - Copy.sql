...
select e.employee_id,e.last_name || ' '||e.first_name, d.department_name, 
    ...
    e.manager_id "manager direct", d.manager_id "Manager departament"        
...
ora?ul ?i tara  n care  lucreaz? angajatii 
...
--corect
select e.last_name || ' '||e.first_name, j.job_title, l.city, c.country_name, job_id
...
--atentie la subcerere cate informatii returneaza
SELECT E.last_name, E.hire_date
FROM EMPLOYEES E
WHERE E.hire_date > ( SELECT E2.hire_date   --subcererea necorelata returneazamai multe info
                    FROM  EMPLOYEES E2 
                    WHERE initcap(e2.last_name)like 'G%' );
--ORA-01427: single-row subquery returns more than one row =>
-- trebuie sa folosim operator de tip multiple-row


--dar:
SELECT distinct E.last_name, E.hire_date
FROM EMPLOYEES E, EMPLOYEES E2   ---self-join
WHERE initcap(e2.last_name)like 'G%'  AND E.hire_date>E2.hire_date;

------------------------------------------------------------------------------                        
Sa se afiseze codul ?i numele angaja?ilor care lucreaz?  n 
acelasi departament cu cel pu?in un angajat al c?rui nume  con?ine litera  t .

Pasul 1: In ce departamente(id+nume) lucreaza angajatii  al c?ror nume con?in litera  t .
...
ex1- Scrieti o cerere pentru a se afisa numele, luna ( n litere) ?i
...
S? se afi?eze numele salaria?ilor ?i numele departamentelor  n 
...
SELECT	employee_id,last_name, e.department_id "dept_id din emp", d.department_id "dept_id din dept",department_name
...
Sa se afiseze codurile de tara,locatie,departament,angajati
inclusiv pt tarile, locatiile si departamentele (din locatiile/tarile respective) 
...
inclusiv pt tarile, locatiile si departamentele (din locatiile respective) in care nu lucreaza nimeni, 
inclusiv locatiile fara tari si departamentele fara locatii
...
--
---- codurile departamentelor n care lucreaz? angaja?i av nd codul job-ului  SA_REP .
...
from locations l, departments d
where l.location_id(+) = d.location_id
order by dep desc; --27 dept (pt ca toate au locatii)

--29 rzultate dupa ce am insirat cele 2 dept fara locatii
...
10.	Se cer codurile departamentelor al c ror nume con ine sirul  re  sau  n care 
lucreaz  angajati av nd codul job-ului  SA_REP .

--codurile departamentelor al c?ror nume con?ine ?irul  re 
--(nu tine cont daca lucreaza angajati in aceste departamente)
...
and department_id is not null;  --toti lucreaza in dept 80  ---29 rez(duplicate toate)
--in total 8 rez
...
--atentie
--numele departamentelor al c ror nume con ine  irul  re  si in care lucreaza angajati
select department_name "Nume departamente", d.department_id
from employees e, departments d
where lower(department_name) like '%re%'
and e.department_id = d.department_id   -- 2 rez
Union
-- numele departamentelor  n care lucreaz  angaja i av nd codul job-ului  SA_REP .
...
10.1.	Se cer codurile si numele departamentelor al c ror nume 
con ine  irul  re  sau  n care 
lucreaz  angaja i av nd codul job-ului  SA_REP .
--corect
--numele departamentelor al c ror nume con ine  irul  re 
...
--ATENTIE!!!!
--functioneaza
--numele si id-ul departamentelor al c ror nume con ine  irul  re 
select department_name "Nume departamente", To_char(department_id)
from departments
where lower(department_name) like '%re%' -- 7 rez
Union
--numele angajatilor si id-ul jobului pt salariatii av nd codul job-ului  SA_REP .
...
select last_name "Nume angajat", job_id, e.department_id -- 29 rez
from employees e, departments d
where lower(job_id) like 'sa_rep'
and e.department_id = d.department_id
order by 1;
...
where lower(department_name) like '%re%'
Union
--numele angaja ilor si id-ul jobului pt salariatii av nd codul job-ului  SA_REP .
...
/*
12.	S  se ob?in  codurile departamentelor  n care nu lucreaz  nimeni 
(nu este introdus nici un salariat  n tabelul employees).
Se cer dou  solu ii (MINUS, NOT IN).
Observa?ie: Operatorii pe mul imi pot fi utiliza i  n subcereri. 
Coloanele care apar  n clauza WHERE a interog rii trebuie s  corespund , 
ca num r  i tip de date, celor din clauza SELECT a subcererii. 
Comenta?i necesitatea trat rii valorilor null  n varianta utiliz rii 
operatorului NOT IN. 

Solu?ie:
*/

--{1,2,3} \ {4,4,4,4,1,1,2,2, null} = {3}
--{1,2,3} \{1,2} = {3}
...
13.	Se cer codurile departamentelor al c ror nume con ine  irul  re  SI  n care 
lucreaz  angaja i av nd codul job-ului  HR_REP . 
...
where lower(department_name) like '%re%' -- 7 rez
Intersect
-- codurile departamentelor  n care lucreaz  angaja i av nd codul job-ului  HR_REP .
...
-- codul departamentului ?i salariul unui angajat care c ?tig? comision.
