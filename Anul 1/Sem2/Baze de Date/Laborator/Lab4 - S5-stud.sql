...
Comanda INSERT are urm?toarea sintax? simplificat?:

INSERT INTO obiect [AS alias] [ (nume_coloan? [, nume_coloan? …] ) ]
...
(metoda implicita de inserare). Determinaþi ca efectele instrucþiunii sã devinã permanente.
...
tabelului lista coloanelor în care se introduc valori (metoda explicita de inserare). 
...
--atentie!
--V1: Creaþi un nou tabel, numit EMP1_***, care va avea aceeaºi structurã ca ºi
--EMPLOYEES, dar nicio înregistrare. 
...
--atnetie, nu exista 252 in employees => pune null
update emp_*** e
set e.salary = (select nvl( a.salary,0)
                from employees a
                where a.employee_id = e.employee_id);
--110 rows updated.  pune 0 la angajatii care nu se gasesc in employees
...
-- 252, 253, 254 vor avea valor de null si asa trebuie sa ramana
...
--set salary =  salary *1.05
select employee_id, salary, salary * commission_pct + salary "venitul total",
    salary * nvl(commission_pct, 0) + salary "venitul total corect", commission_pct
from emp_***;
100	25200
101	17850
102	17850
...
and lower(last_name) = 'grant'; --199 2600 50
...
from emp_***
where lower(first_name) = 'douglas'
and lower(last_name) = 'grant'; --199 4600 20
 rollback;
...
--atentie
delete from DEPT_***
 where department_id not in (select department_id    --nu apare dept 300 ca fiind un dept in care lucreaza angajati
                           from employees
                           where department_id is not null);
...
delete from emp_***
where employee_id =255; --1 rows deleted. 
commit;
