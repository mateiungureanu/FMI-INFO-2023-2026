-----Lab 7 - sapt 9 +10
--Continuare GROUP BY, subcereri necorelate in clauza de FROM -sapt 9
---fct decode -sapt 10
...
--Pas 0: sa se obtina codul acestor departamente si salariul mediu pe departament.(pt fiecare departament)

SELECT e.DEPARTMENT_ID,round(avg(E.SALARY))  
FROM EMPLOYEES E
GROUP BY e.DEPARTMENT_ID; --12 rez


Pas 2: S? se afi?eze maximul dintre salariile medii pe departamente.
...
-- V2 cu with
with
aux as (SELECT D.DEPARTMENT_ID,D.DEPARTMENT_NAME,round(avg(E.SALARY)) media, min(salary)
        FROM EMPLOYEES E,DEPARTMENTS D
        WHERE E.DEPARTMENT_ID=D.DEPARTMENT_ID
        GROUP BY D.DEPARTMENT_ID,D.DEPARTMENT_NAME),
tab_maximul as (
...
c) in cate dept lucreaza mai putin de 4 ANG?
a)
...
--sau cu with
with 
tab_infor as(SELECT  D.DEPARTMENT_ID, D.DEPARTMENT_NAME, count(*) nr
            FROM EMPLOYEES E,DEPARTMENTS D
            WHERE E.DEPARTMENT_ID=D.DEPARTMENT_ID
            GROUP BY D.DEPARTMENT_NAME, D.DEPARTMENT_ID),
            
tab_nr_max as (select max(nr) maximul
                from tab_infor)

select *
from tab_infor
where nr = (Select maximul
            from tab_nr_max);
...
WHERE E.DEPARTMENT_ID IN   --folosim IN pt ca pot exista mai multe dept cu nr maxim de angajati
                      ...
                      HAVING COUNT(*) = (SELECT max(count(*))
                                          FROM EMPLOYEES E
                                          GROUP BY e.DEPARTMENT_ID)); --45 ang
...
respectiv si salariul total pentru job-ul respectiv pe 
...
v2:
SELECT job_id, SUM(DECODE(department_id, 30, salary,0)) Dep30,
...
SELECT job_id, 
    ...
    FROM employees
    WHERE department_id = 30
    AND job_id = j.job_id) Dep30,
            (SELECT NVL(SUM(salary),0)
            FROM employees
            WHERE department_id = 50
            AND job_id = j.job_id) Dep50,
                (SELECT NVL(SUM(salary),0)
                FROM employees
                WHERE department_id = 80
                AND job_id = j.job_id) Dep80,
      (select nvl(sum(salary),0)
      from employees
      where job_id =j.job_id) total
FROM jobs j; --20 rez
--apare si RLX	0	0	0	(un job pe care nu lucreaza nimeni)



--13.	 S? se creeze o cerere prin care s? se afi?eze num?rul total de angaja?i ?i,
...
--nu
SELECT COUNT(*) TOTAL,
count(DECODE(TO_CHAR(hire_date,'yyyy'),1997,1,0)) "AN 1997",--atentie
SUM(DECODE(TO_CHAR(hire_date,'yyyy'),1998,1,0)) "AN 1998",
SUM(DECODE(TO_CHAR(hire_date,'yyyy'),1999,1,0)) "AN 1999",
SUM(DECODE(TO_CHAR(hire_date,'yyyy'),2000,1,0)) "AN 2000"
FROM employees;
...
--corect:
SELECT COUNT(*) TOTAL,
count(DECODE(TO_CHAR(hire_date,'yyyy'),1997,198428657)) "AN 1997",
SUM(DECODE(TO_CHAR(hire_date,'yyyy'),1998,1,0)) "AN 1998",
SUM(DECODE(TO_CHAR(hire_date,'yyyy'),1999,1,0)) "AN 1999",
SUM(DECODE(TO_CHAR(hire_date,'yyyy'),2000,1,0)) "AN 2000"
FROM employees;
...
--NU---manager_id din employees nu este seful de departament, este seful direct al angajatului
SELECT department_id,MANAGER_ID ,SUM(salary) suma 
FROM employees
GROUP BY department_id, MANAGER_ID
ORDER BY 1, 2;

SELECT d.department_id, d.department_name, a.suma, D.MANAGER_ID  -- sau a.department_id in loc de d.department_id
...
SELECT d.department_id,d.MANAGER_ID ,SUM(salary) suma 
FROM employees e, departments d
WHERE d.department_id = e.department_id(+)
GROUP BY d.department_id, d.MANAGER_ID
ORDER BY 1, 2; --27 rez  De ce funtioneaza? Este buna grupare? 
--Un departament are un singur manager de departament?
...
--commit;
--avem deja un job de test introdus
--Test	Testadministration Vice President	22500	15000	nu lucreaza nimeni
...
select j.job_id, job_title, (min_salary+max_salary)/2, max_salary - min_salary, 
       nvl(to_char(aux.media_reala), 'nu lucreaza nimeni') media_real, 
       (min_salary+max_salary)/2- nvl(aux.media_reala,0) dif,
       (min_salary+max_salary)/2-aux.media_reala dif_cu_null
