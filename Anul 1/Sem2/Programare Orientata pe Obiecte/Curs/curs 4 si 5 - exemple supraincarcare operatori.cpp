...
/********** op= neaparat de supraincarcat *****************/

class student{
    const int an;
    string nume;
public:
    student(string s, int a) : an(a){nume = s;}
    void afis(){cout<<an<<" "<<nume<<endl;}
...
class materie
{
    ...
    string denumire;
public:
    materie(string A, student&ss) : s(ss) {denumire = A;}
    void afis(){cout<<denumire<<" "; s.afis();}
...
{
    student A("Iona",1968);
    student B("Harap_Alb",1876);
/*    A = B; /// const au nevoie de supraincarcare op=
    A.afis();
    B.afis();
    student C = A;
    C.afis();
*/
    materie X("Algebra",A);
    X.afis();
    materie Y("Geometrie",B);
    Y.afis();
    materie Z = Y;
    Z.afis();
    materie T("Analiza",A);
    T = Y; // date membre referinta necesita supraincarcare op=
    T.afis();
}
...
{
///    in>>(Test&)ob; /// downcasting
    in>>dynamic_cast<Test&>(ob);
...
/*** acces data membra, var locala si var globala ***/
/*
int x = 10;
class Test
{
...
public:
    Test(){ x = 20;}
    void afis(int x){
        cout<<"param functie: "<<x<<endl;
        cout<<"data membra: "<<Test::x<<" "<<this->x<<endl;
        cout<<"global: "<<::x<<endl;
    }
};
